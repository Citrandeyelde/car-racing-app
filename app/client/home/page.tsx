        ...
        {loading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {[...Array(8)].map((_, i) => (
              <div
                key={i}
                className="h-80 bg-zinc-900 animate-pulse rounded-lg border border-zinc-800"
              ></div>
            ))}
          </div>
        ) : (
              ...
              >
                ...
                <div className="aspect-square overflow-hidden bg-zinc-800 relative">
                  ...
                  {item.stock <= 0 && (
                    <div className="absolute inset-0 bg-black/60 flex items-center justify-center">
                      <span className="bg-red-600 px-3 py-1 text-xs font-bold uppercase -rotate-12">
                        Agotado
                      </span>
                    </div>
                  )}
                ...
                <div className="p-5">
                  ...
                  <p className="text-zinc-500 text-xs line-clamp-2 h-8 mb-4">
                    {item.descripcion ||
                      "Especificaciones técnicas no disponibles."}
                  </p>
                  ...
                  <div className="flex items-center justify-between border-t border-zinc-800 pt-4">
                    ...
                    <button
                      ...
                      onClick={() =>
                        addToCart({
                          id: String(item.id_producto), // Convertimos a string para consistencia
                          name: item.nombre,
                          price: item.precio,
                      ...
                      className={`px-4 py-2 rounded-lg font-bold uppercase italic transition-all duration-300 ${
                        item.stock > 0
        ...
        {!loading && productos.length === 0 && (
          <div className="text-center py-20 border-2 border-dashed border-zinc-800 rounded-3xl">
            <p className="text-zinc-500 uppercase tracking-widest">
              No hay piezas disponibles en el inventario actual
            </p>
          </div>
        )}
      </div>
